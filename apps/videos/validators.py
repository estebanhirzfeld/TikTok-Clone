...
def validate_thumbnail_image_format(value):
    try:
        ...
        if image.format.upper() not in allowed_extensions:
            raise ValueError(
                _(
                    "Invalid image format. Supported formats are JPEG, JPG, WEBP, and PNG."
                )
            )
...
def validate_video_format(value):
    ...
    if file_extension not in allowed_extensions:
        raise ValidationError(
            _(
                "Invalid video file format. Only .MP4, .AVI, .MKV, .MOV files are allowed."
            )
        )
...
def validate_video_size(value):
    ...
    if value.size > max_size:
        raise ValidationError(
            _("Video size exceeds the maximum allowed size of 50 MB.")
        )
