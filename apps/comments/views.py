...
from .emails import (
    ...
    send_video_comment_notification,
)
from .models import Comment
from .pagination import CommentPagination
from .permissions import IsOwnerOrReadOnly
from .renderers import CommentRepliesJSONRenderer, CommentsJSONRenderer
...
class CommentListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = Comment.objects.filter(
            Q(video__user__profile__is_private=False)
            | Q(  # Comments where video user profile is not private
                video__user__profile__followers=self.request.user.profile
            )
            | Q(  # Comments where video user is followed by request user
                video__user=self.request.user
            )  # Comments where video user is the request user
        )
    ...
    def perform_create(self, serializer):
        ...
        if (
            not video_user.is_private
            or video_user.check_follower(user)
            or video.user == user
        ):
            ...
            raise PermissionDenied(
                "You do not have permission to comment on this video."
            )
...
class CommentRepliesListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        parent_response = get_object_or_404(Comment, id=comment_id)
        serializer.save(
            user=user, video=parent_response.video, parent_response=parent_response
        )
        send_comment_reply_notification(
            user, parent_response.user, parent_response.video
        )
